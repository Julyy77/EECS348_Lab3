...
void sales_summary(float sales_data[MONTH_NUM]){
    float min_sale = sales_data[0]; 
    float max_sale = sales_data[0];
    float sum_sale = 0.0; 
    float average_sale = 0.0;
    int min_month = 0;
    int max_month = 0;
    
    for (int i = 0; i < MONTH_NUM; i++) {
        if (sales_data[i] < min_sale) {
            min_sale = sales_data[i];
            min_month = i;
        }
        if (sales_data[i] > max_sale) {
            max_sale = sales_data[i];
            max_month = i;
        }
        sum_sale += sales_data[i];
    }

    average_sale = sum_sale / 12;

    printf("Minimum sales: $%.2f (%s)\n", min_sale, sales_data[min_month]);
    printf("Maximum sales: $%.2f (%s)\n", max_sale, sales_data[max_month]);
...
void six_moving_average(float sales_data[MONTH_NUM]){
    ...
    for (int i = 0; i < MONTH_NUM - 5; i++) {
        sum = (sales_data[i] + sales_data[i + 1] + sales_data[i + 2] + sales_data[i + 3] + sales_data[i + 4] + sales_data[i + 5]);
        avg = sum / 6.0;
